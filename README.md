...
中国地理位置信息, 银联卡跨行业务地区代码标准，人民共和国县以上行政区划代码。并做了Id映射，方便使用。
...
- ### 银联卡地区代码-行政代码映射.json
    ...
    这是将银行的地名数据和行政代码地名数据匹配后的json. 
    值得注意的是，由于此json使用银联卡去匹配行政区域代码,所以匹配的结果将会可能出现以下情况：
    
    - 两个名称不匹配的情况，原因是银联地区码匹配到的行政区域名称可能已经改名或者从从县升到市（共有3个）
    - 某些地名行政区域已经撤销，则将code绑定到上级，比如县级撤销了则绑定到市下面（共有10个）
    - 某些新增的行政区域，则将code绑定到上级，比如县级撤销了则绑定到市下面（共有13个）
    ...
    {
        childs: [], 
        code: "12", // 行政区划代码里面的 code
        codeName: "天津市",  // 行政区划代码里面的 name
        id: "1100", // 银联地区代码的 id
        name: "天津市",  // 银联地区代码的名称
        pid: "-1",
        pname: "全国"
